...
const AddPlayerLinkModal = ({ isOpen, onClose }: {
  ...
  const { selectedTeam, setSelectedTeam } = useTeamStore() as { selectedTeam: Team, setSelectedTeam: (team: Team) => void }

  const handleAddLink = () => {
    if (link.length === 0) {
      toast.error("Input player link");
    } else if (selectedTeam?.players?.some((player: Player) => player.link === link)) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} containerClassName="bg-white dark:bg-boxdark p-4 w-[450px]">
      <span className="font-semibold text-black dark:text-white">Add Player Link</span>
      <input
        name='link'
        type="text"

...
const PriorityList = () => {
  const { selectedPlayer, setSelectedPlayer, priorityPlayers, setPriorityPlayers } = usePlayerStore() as {
  ...
  const handleSelectRow = (index: number) => {
    ...
    if (newSelectedRows.size === 2) {
      ...
      toast.success(`Players at positions ${firstIndex + 1} and ${secondIndex + 1} have been swapped.`);
    }
  };
  ...
  return (
    <div className="min-h-full max-h-[calc(100vh-8rem)] overflow-y-auto p-4 bg-transparent border border-stroke dark:bg-boxdark dark:border-strokedark">
      <div className="flex justify-between items-center gap-4">
        <span className="text-2xl text-black dark:text-white font-medium">Priority List</span>
        <button
      ...
      {selectedPlayer && (
        <div className="mt-4">
          <span className="text-lg">Selected Player: {selectedPlayer.name}</span>
      ...
      <table className="min-w-full table-auto bg-white dark:bg-bodydark border border-stroke dark:border-strokedark rounded-md mt-4">
        <thead className="bg-gray dark:bg-boxdark">
          <tr className="bg-gray-200 dark:bg-boxdark">
            <th />
            <th className="px-6 py-3 text-left text-black dark:text-white">Round</th>
            <th className="px-6 py-3 text-left text-black dark:text-white">Player Name</th>
          </tr>
        </thead>
        <tbody>
          {priorityPlayers.map((player, index) => (
            <tr
              key={index}
              className={`${index % 2 === 1 ? 'bg-white dark:bg-bodydark' : 'bg-whiter dark:bg-bodydark1'} hover:bg-gray-100 hover:bg-stroke dark:hover:bg-white cursor-pointer dark:text-strokedark`}
